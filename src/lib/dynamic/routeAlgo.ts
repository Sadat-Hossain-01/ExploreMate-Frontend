...
async function getDayitemFromDest(
    ...
    for (let i: number = 0; i < eligibleActivities.length; i++) {
        ...
        cost +=
            (plan.buddy_count *
                (eligibleActivities[i].minCost +
                    eligibleActivities[i].maxCost)) /
            2;
    ...
    for (let i: number = 0; i < eligibleEvents.length; i++) {
        ...
        cost +=
            (plan.buddy_count *
                (eligibleEvents[i].minCost + eligibleEvents[i].maxCost)) /
            2;
    ...
    let dayitem: DayItem = {
        ...
        description:
            description +
            ". Travel time: " +
            getStringFromSeconds(duration) +
            ". Travel cost: $" +
            roundToTwo((distance * transportCost) / 1000),
        estimated_cost: roundToTwo(
            dest.estimated_cost + cost + (distance * transportCost) / 1000
        ),
    };
    return dayitem;
}
...
export async function routeAlgo(plan: Plan) {
    ...
    while (dayCount <= plan.duration) {
        let flag: boolean = false;
        let lunchDone: boolean = false;
        let dinnerDone: boolean = false;
        let day: Day = {
            date:
                current.getDate() +
                "-" +
        ...
        if (firstHotel.comp_breakfast) {
            ...
            dayitem = {
                ...
                description:
                    ...
                    roundToTwo((distance * transportCost) / 1000),
                estimated_cost: roundToTwo(
                    restaurant.breakfast * plan.buddy_count +
                        (distance * transportCost) / 1000
                ),
            };
        ...
        while (true) {
            ...
            if (
                convertTimeToNumber(newdayitem.end_time) + duration / 3600 >
                14.5
            ) {
                break;
            } else {
        ...
        if (flag) {
            if (plan.destinations.length == 0) {
                dayitem = {
                    name: "End of the trip",
                    start_time: convertNumberToTime(time),
                    end_time: convertNumberToTime(time),
                    lat: lastPosLat,
                    lng: lastPosLng,
                    description: "",
                    estimated_cost: 0,
            ...
            if (time + clac.duration / 3600 > 15.5) {
                lunchDone = true;
                let restaurant: Restaurant = await getNextRestaurant(
                    lastPosLat,
                    lastPosLng,
                ...
                dayitem = {
                    ...
                    description:
                        ...
                        getStringFromSeconds(duration),
                    estimated_cost: roundToTwo(
                        restaurant.lunch * plan.buddy_count +
                            (distance * transportCost) / 1000
                    ),
                };
            ...
            city_destinations = await destinationCity(
                firstHotel,
                plan.destinations
            );
            ...
            dayitem = {
                ...
                lng: firstHotel.lng,
                description:
                    "Finish touring the city and check in to " +
                    firstHotel.name +
                    ". Travel time: " +
                    getStringFromSeconds(clac.duration) +
                    ". Travel cost: $" +
                    roundToTwo((clac.distance * transportCost) / 1000),
                estimated_cost: roundToTwo(
                    (clac.distance * transportCost) / 1000
                ),
            };
        ...
        if (time > 20) {
            ...
            dayitem = {
                ...
                description:
                    ...
                    roundToTwo((distance * transportCost) / 1000),
                estimated_cost: roundToTwo(
                    restaurant.dinner * plan.buddy_count +
                        (distance * transportCost) / 1000
                ),
            };
            time = time + duration / 3600 + 1;
            ...
            dayitem = {
                ...
                description:
                    "Return to " +
                    firstHotel.name +
                    " and take in the local scenery. Travel time: " +
                    getStringFromSeconds(clac.duration),
                estimated_cost: roundToTwo(
                    (clac.distance * transportCost) / 1000
                ),
            ...
            lastPosLat = firstHotel.lat;
            lastPosLng = firstHotel.lng;
            time = Math.max(time + clac.duration / 3600, 20);
            day.items.push(dayitem);
        }
        if (!dinnerDone) {
            let restaurant = await getNextRestaurant(
                lastPosLat,
                lastPosLng,
                prefRestaurants,
                city_restaurants,
            ...
            dayitem = {
                name: restaurant.name,
                start_time: convertNumberToTime(time + clac.duration / 3600),
                end_time: convertNumberToTime(time + clac.duration / 3600 + 1),
                lat: restaurant.lat,
                lng: restaurant.lng,
                description:
                    "Take dinner in " +
            ...
            time = time + clac.duration / 3600 + 1;
            day.items.push(dayitem);
            lastPosLat = restaurant.lat;
            lastPosLng = restaurant.lng;
        }
